...
class tree {
    ...
    template<typename iter> iter Merge(const T& x, iter pos1, iter pos2)
    {
      iter pos1N, pos2N;

      iter it = insert(pos1, x);
      pos1N = append_child(it, x);
      pos2N = append_child(it, x);

		  move_ontop(pos1N, pos1);
		  move_ontop(pos2N, pos2);
